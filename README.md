...
### POST `/api/price/schedule-alter/reserve-slot-alter`
...
В случаи успешного бронирования нужно будет сохранить crm_appointment_alter_id - он понадобится при подтверждении.
...
| `analysis_id` | int    | Идентификатор анализа (`erp.analysis.id`)           |
| `dt`          | string | Дата в формате `Y-m-d`                              |
| `time_start`  | string | Время начала в формате `H:i:s`                      |
...
| `p_phone`                  | string | Телефон пациента (только цифры)           |
